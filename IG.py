...
{K}[{P}•{K}]{P}+++++++++++++++++++++++++++++++++++++++++
{H}[{P}>{H}]{P} Autor : GF-Elite
{H}[{P}>{H}]{P} Ig : @andika_12124123
{H}[{P}>{H}]{P} Fb : fb.com/kananda.widiantarakananda.50
{H}[{P}>{H}]{P}whatsapp: 081375182098
{H}[{P}>{H}]{B}Waktu EXPIRED : 22 may 2055
{K}[{P}•{K}]{P}+++++++++++++++++++++++++++++++++++++++++
""")
# Login Cookie
def ___login___():
    ...
    if ___cookie in ['open', 'Open', 'OPEN']:
        print(f"{K}[{P}!{P}]{K} Anda Akan Diarahkan Ke whatsapp, Silahkan Ikuti Cara Untuk Mendapatkan Cookie...");sleep(3);os.system('xdg-open https://wa.me/+6281375182098');exit()
    elif ___cookie in ['', ' ']:
        ...
        try:
            ...
            print(f"{H}[{P}*{H}]{P} Selamat datang:{K} {___roz['full_name']}");___follow___()
        except (AttributeError, KeyError):
            exit(f"{P}[{M}!{P}]{M} Pastikan Cookie Sudah Benar jangan salah broo")
...
def ___follow___():
    try:
        ...
        ___teks = random.choice(['Hallo Bang 😍','Hai Bro Apa Kabar 😎','Izin Pake Scriptnya 😁','Mantap Bang 😘','Programmer Bang 🤔','Salam Kenal Bang 🤗','I Love You ❤️'])
...
def ___menu___():
    try:
        ...
        print(f"{B}[{P}*{B}]{P} Selamat Datang :{K} {___roz['full_name']}")
        print(f"{B}[{P}*{B}]{P} Pengguna :{K} {___roz['username']}")
