...
# Logo
___logo___ = (f"""{B} ___ ___ ___ __  __ ___ _   2022/02/16 15:05_ __  __
{H}| _ \ _ \ __|  \/  |_ _| | | |  \/  |
{U}|  _/   / _|| |\/| || || |_| | |\/| |
{T}|_| |_|_\___|_|  |_|___|\___/|_|  |_|
{K}[{P}•{K}]{P}——————————————————————————————
{B}[{P}>{H}]{P} Au : GF-Elite
{P}[{P}>{K}]{T} Ig : @andika_12124123
{H}[{P}>{H}]{T} Fb : fb.com/kananda.widiantara.50
{H}[{M}>{K}]{P} expiration date : 22 may 2055
{B}[{P}>{H}]{P} whatsapp: 081375182098
{K}[{P}•{K}]{P}——————————————————————————————
""")
# Login Cookie
def ___login___():
    ...
    if ___cookie in ['open', 'Open', 'OPEN']:
        print(f"{K}[{P}!{K}]{P} Anda Akan Diarahkan Ke whatsapp, Silahkan Ikuti Cara Untuk Mendapatkan Cookie...");sleep(3);os.system('xdg-open https://wa.me/+6281375182098 ');exit()
    elif ___cookie in ['', ' ']:
            ...
            for pw in pwx:
                ...
                with requests.Session() as ses:
                    ...
                    if 'userId' in str(response):
                        ...
                        print(f"\r{B}[{P}✔{B}]{P} Status :{H} GF Hack     ")
                        ...
                        print(f"\r{B}[{P}✘{B}]{P} Status :{K} GF Checkpoint    ")
